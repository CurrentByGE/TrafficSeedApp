...
gulp.task('elements', function () {
  return styleTask('elements', ['**/*.scss']);
});
...
gulp.task('copy', function () {
  ...
  var nginxScripts = gulp.src('app/nginx/scripts/**/*')
    .pipe(gulp.dest('dist/nginx/scripts'));

  // Temporary fix until the openresty buildpack is fixed. Need to sqush the root and
  // server conf files down
  var nginxConf = gulp.src(['nginx.conf']);

  // var nginxConfigSrcs = ['root.conf', 'http.conf', 'server.conf'];
  // nginxConfigSrcs.forEach(function (src) {
  //   var fn = path.join('app', 'nginx', 'conf', src);
  //   if (fs.existsSync(fn)) {
  //     nginxConf.pipe($.replace('include <%= ENV["APP_ROOT"] %>/nginx/conf/' + src + ';', fs.readFileSync(fn)));
  //   }
  // });
  nginxConf.pipe(gulp.dest('dist'));

  var userLua = gulp.src(['user.lua'])
    .pipe(gulp.dest('dist/'));

  return merge(app, bower, elements, vulcanized, swBootstrap, swToolbox, nginx, nginxConf, nginxScripts, userLua)
...
function fakeRestServices(req, res, next) {
  if (req.url.match(/\/services\/v1\/assets\/[^\/]+\/events(.*)TFEVT-D(.*)/)) {
    ...
    res.end(fs.readFileSync('app/elements/json-data/locations.json'));
  } else if (req.url.match(/\/services\/v1\/assets/)) {
    res.end('{"_links":{"self":{"href":"/v1/assets"},"search":{"href":"/v1/assets/search{?q,bbox,page,size}","templated":true}}}');
  } else if (req.url.match(/\/services\//)) {
    res.end('{"_links":{"assets":{"href":"/v1/assets"},"locations":{"href":"/v1/locations"}}}');
  } else if (req.url.match(/\/oauth\/token(.*)/)) {
    res.end(fs.readFileSync('app/elements/json-data/getToken.json'));
  } else {
    next();
  }
}
...
gulp.task('serve', ['styles', 'elements', 'images'], function () {
  browserSync({
    ...
    server: {
      baseDir: ['.tmp', 'app'],
      middleware: [ historyApiFallback(), fakeRestServices ],
      routes: {
