...
function M.getAuthenticationCode()
  ...
  if uaa_uri ~= nil then
    local query_string = ngx.encode_args({
      ...
      redirect_uri = 'https://' .. ngx.var.host .. '/oauth-callback',
...
function exchangeToken(code)
  ...
  local response = ngx.location.capture(
      '/_internal/_access_token', {
          ...
          body = 'grant_type=authorization_code&code='..code..'&redirect_uri=https%3A%2F%2F'..ngx.var.host..'%2Foauth-callback&state='..state
      }
  )
